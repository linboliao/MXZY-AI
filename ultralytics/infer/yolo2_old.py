...
class Result:
    def __init__(self, opt):
        ...
        self.color_dict = {'prostate': [0, 255, 0], 'cancer': [255, 0, 0], 'burn': [0, 0, 255], 'vessel': [255, 255, 0],
                           'epithelium': [255, 0, 255], 'ganglion': [0, 255, 255]}
    ...
    def slides(self):
        if self.slide_list and len(self.slide_list) > 0:
            ...
            slides = []
            exts = ['.svs']
            for root, dirs, files in os.walk(self.slide_dir):
                for file in files:
...
class GeoResults(Result):
    ...
    def cal_rate(self, coords, labels):
        ...
        for coord, label in zip(coords, labels):
            ...
            y1 = max(y_list)
            y2 = max(y_list)
            if label == 'cancer' or label == 1:
                cancer_area += (x2 - x1) * (y2 - y1)
            elif label == 'prostate' or label == 0:
    ...
    def post_process(self, coords, labels, confs, base):
        ...
        #     json.dump(data, f, indent=4, ensure_ascii=False)  # 格式化输出便于阅读


class MultiGeoResults(GeoResults):
    def process(self, slide):
        base = os.path.splitext(os.path.basename(slide))[0]
        if os.path.exists(os.path.join(self.output_dir, f"{base}.level0.geojson")):
            return
...
class MdsResults(Result):
    def __init__(self, opt):
        super().__init__(opt)
        self.color_dict = {'prostate': '4278255615', 'cancer': '4294901760', 'burn': [0, 0, 255],
