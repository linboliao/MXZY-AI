        ...
        for count, (batch, coords) in enumerate(pbar):
            with torch.no_grad():
                ...
                for idx, model in enumerate(self.models):
                    results.append(
                        model(batch, device='0', agnostic_nms=True, iou=0.4, verbose=False, conf=0.2, half=False))

                result = []  # 存储当前批次每个图像的最终结果
                for idx in range(batch.size(0)):  # 遍历批次中的每个图像
                    # 第一阶段：收集四个模型的结果
                    xyxy_all, conf_all, cls_all = [], [], []
                    boxes = [item[idx].boxes for item in results]  # 四个模型的Boxes对象列表
...
def save_area(area_data, csv_path, key_column='slide_id'):
    ...
    if os.path.exists(csv_path):
        existing_df = pd.read_csv(csv_path, dtype={'slide_id': str})
...
def main(args):
    ...
    for index, slide_id in enumerate(exist_idxs):
        ...
        dataset = Whole_Slide_Bag_FP(file_path=h5_file_path, wsi_path=slide_file_path, pretrained=True,
                                     custom_transforms=custom_transformer, fast_read=True)
        ...
        area_data.append(
            {'slide_id': slide_id, 'area': f'{converter.malignant_area / converter.tissue_area * 100:.4f}%'})
