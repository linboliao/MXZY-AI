...
def extract_features_parallel(args):
    ...
    csv_paths = generate_cls_csv(args.output_dir, coord_dir, args.wsi_dir, part)
    csv_paths = [p for p in csv_paths if os.path.exists(p)]
    if not csv_paths: return False

    with concurrent.futures.ProcessPoolExecutor(max_workers=part) as executor:
...
#         os.path.join(path, 'infer/yolo2x2.py'),
#         "--model", 'yolo',
#         "--task", 'detect',
#         "--data_coors_dir", coord_dir,
#         "--data_slide_dir", args.wsi_dir,
...
def run_yolo(args):
    ...
    yolo_cmd = [
        os.path.join(conda_path, "ultralytics/python.exe"),
        os.path.join(path, 'infer/yolo2x3.py'),
    ...
    return run_command(path, yolo_cmd, f"YOLO检测")


def run_yolo_parallel(args):
    """YOLO目标检测任务"""
    parts = 4
    csvs = generate_yolo_csv(args.csv_path, args.output_dir, num_parts=parts)
    with concurrent.futures.ProcessPoolExecutor(max_workers=parts) as executor:
...
def run_medical_image_pipeline(wsi_dir: str, output_dir: str, slide_list=None, patch_level: int = 0,
    ...
    try:
        ...
        phase1_tasks = [run_cls, run_gleason]
        phase1_names = ["癌症诊断", "Gleason诊断"]
...
def _generate_result_files(output_dir: str) -> None:
    ...
    for slide_id, pred, votes_0 in zip(cancer_df['slide_id'], cancer_df['prediction'], cancer_df['votes_0']):
        ...
        if pred == 0 or num == 0:
            ...
            typ = 'Malignant'
        conf = 'weak' if votes_0 in [3] else 'strong'
...
wsi_group.add_argument("--normal", type=bool, default=False, help="归一化")
