...
if __name__ == '__main__':
    ...
    for index, bag_candidate_idx in enumerate(exist_idxs):
        ...
        if args.model == 'virchow':
            class_token = features[:, 0]    # size: seq_len x 1 x 1280
            patch_tokens = features[:, 1:]  # size: seq_len x 256 x 1280
            features = torch.cat([class_token, patch_tokens.mean(1)], dim=-1) # size: seq_len x 1 x 2560
