...
def seg_and_patch(
    ...
    for i in range(total):
        ...
        if csv_path is not None:
            # 从 CSV 读取的 slide 已经是完整路径
            full_path = slide
        else:
            # 从遍历 source 得到的 slide 只是相对路径，需要拼接
            full_path = slide
        try:
        ...
        if patch:
            ...
            para = {"patch_level": patch_level, "patch_size": patch_size, "step_size": step_size, "save_path": patch_save_dir}
            with open('patch_params.txt', 'a', encoding='utf-8') as f:
                f.write("=" * 50 + "\n")
...
parser.add_argument("--csv_path", type=str, default=None, help="patch_size")
...
def main(args):
    ...
    if args.preset:
        preset_df = pd.read_csv(os.path.join("PrePATH/presets", args.preset))
        for key in seg_params.keys():
